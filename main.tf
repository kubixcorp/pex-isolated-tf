...
resource "null_resource" "push_image" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      aws ecr get-login-password --region ${data.aws_region.virginia.name} --profile fmoralesIsolated | docker login --username AWS --password-stdin ${aws_ecr_repository.nginx.repository_url}
      echo "FROM --platform=linux/amd64 nginx:1.21.3" > Dockerfile
      docker build -t ${aws_ecr_repository.nginx.repository_url}:1.21.3 .
      docker push ${aws_ecr_repository.nginx.repository_url}:1.21.3
      rm Dockerfile
    EOF
  }
...
resource "aws_ecs_task_definition" "ecs_task_def_virginia" {
  ...
  container_definitions = jsonencode([
    {
      name  = "app"
      image = "${aws_ecr_repository.nginx.repository_url}:1.21.3"
      portMappings = [
        {
          containerPort = 80
          hostPort      = 80
  ...
  depends_on = [time_sleep.wait_30_seconds, aws_iam_role.ecs_task_execution_role,
